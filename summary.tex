...
\subsubsection{Internet Protocol (IPv4 only)}
\paragraph{}This is the backbone of the internet, it allows hosts to connect to eachother. Routers uses IP addresses to route IP packets from
network to network.

\subsubsection{Ethernet}
\paragraph{}Ethernet is a protocol at the Data link layer of the OSI model. It actually contains two layers:
\begin{description}
	\item [802.3 Media Access Layer (MAC)] The physical part of this layer.
	\item [802.2 Logical link Control (LLC)] The software part of this layer.
\end{description}
\paragraph{}Ethernet does the following things on this layer:
\begin{description}
	\item [Collision Detection (CSMA/CD)] If a collision occurs it will sets a timer and wait for x amount of miliseconds.
	\item [Encapsulation of the Network layer] Note that this encapsulation has a header and a trailer.
	\item [Ethernet addressing] A unique address for a single device on a LAN network. Switches uses MAC addresses to switch
	packets between divices instead of IP addresses.
\end{description}

\paragraph{}An ethernet address is 48 bits long. The first 24 bits are for the \textbf{Organizationally Unique Identifier (OUI)}, the last 24 bits
...
\paragraph{}
A WAN is a network that connects multiple LANs together. It uses PPP, Frame Relay, or HDLC at layer 1 or 2 depending on the protocol.
